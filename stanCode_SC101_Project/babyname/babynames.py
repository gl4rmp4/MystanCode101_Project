...
def add_data_for_name(name_data, year, rank, name):
    ...
    """
    # check the name and year in data
    ...
    with open(filename, 'r') as f:
        for line in f:
            if first_row_gate:
                ...
                babynameinfo = line.split(',')
                rank = babynameinfo[0].strip()
                name1 = babynameinfo[1].strip()
                name2 = babynameinfo[2].strip()
...
def read_files(filenames):
    ...
    for i in range(len(filenames)):
        add_file(name_data,filename=filenames[i])
    return name_data
